...
```javascript
import Vue from 'vue'
import PyStep from 'element-steps'
...
```javascript
import Vue from 'vue'
import { PySteps, PyStep } from 'element-steps'

Vue.component('py-steps', PySteps)
Vue.component('py-step', PyStep)
```
...
| icon | 图标 | Pyement Icon 提供的图标，如果要使用自定义图标可以通过 slot 方式写入 | string | — |
